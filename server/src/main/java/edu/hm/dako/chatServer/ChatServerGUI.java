...
public class ChatServerGUI extends Application implements ChatServerGuiInterface {
    ...
    private void startChatServerWithAuditLogServer(String implType, int serverPort, int sendBufferSize,
        ...
        ChatServerImplementationType serverImpl = ChatServerImplementationType.TCPAdvancedImplementation;
    ...
    private int readSendBufferSize() {
        ...
        if (item.matches("[0-9]+")) {
            ...
            if ((iSendBufferSize <= 0)
                    || (iSendBufferSize > Integer.parseInt(MAX_SENDBUFFER_SIZE))) {
                startable = false;
    ...
    private int readReceiveBufferSize() {
        ...
        if (item.matches("[0-9]+")) {
            ...
            if ((iReceiveBufferSize <= 0)
                    || (iReceiveBufferSize > Integer.parseInt(MAX_RECEIVEBUFFER_SIZE))) {
                startable = false;
     ...
     * @param message Meldung fuer Blidschirmanzeige
     */
